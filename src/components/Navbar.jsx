...
const STUDY_LINKS = [
  { to: '/study-in-germany', icon: 'school', label: 'Study in Germany' },
  { to: '/blocked-account-germany', icon: 'account_balance', label: 'Blocked Account Germany' },
]
...
export default function Navbar() {
  ...
  const dropdownTrigger = (active) =>
    `text-sm font-semibold transition-colors cursor-default select-none flex items-center gap-0.5 ${active ? 'text-primary' : 'text-gray-700 hover:text-primary'}`
      ...
      <div className="bg-gray-900 text-gray-300 text-xs py-2 hidden md:block">
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          <div className="flex items-center gap-5">
            ...
            <a href="https://wa.me/4915236738625" target="_blank" rel="noopener noreferrer"
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-16">
          ...
          <ul className="hidden md:flex items-center gap-6 list-none m-0 p-0">
            ...
            <li className="relative group py-2">
              <span className={dropdownTrigger(false)}>
                Study
                <span className="material-icons-round text-base leading-none" style={{ fontSize: 16 }}>expand_more</span>
              </span>
              <div className="absolute left-0 top-full mt-0 w-56 bg-white border border-gray-100 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                <div className="p-2 space-y-0.5">
                  {STUDY_LINKS.map(item => (
                    <Link key={item.to} to={item.to}
                      className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg hover:bg-primary/5 hover:text-primary transition-colors group/item">
                      <span className="material-icons-round text-primary text-base group-hover/item:text-primary">{item.icon}</span>
                      <span className="text-sm font-semibold text-gray-800 group-hover/item:text-primary">{item.label}</span>
                    </Link>
                  ))}
                </div>
              </div>
            ...
            <li className="relative group py-2">
              <NavLink to="/products" className={navLinkClass}>
                Products
                <span className="material-icons-round text-base align-middle" style={{ fontSize: 16 }}>expand_more</span>
              </NavLink>
                  ...
                  <div className="grid grid-cols-2 gap-x-2 gap-y-1">
                    {categories.map(cat => (
                      <Link key={cat.slug} to={`/products?category=${cat.slug}`}
                        className="block px-3 py-2.5 text-sm font-medium text-gray-700 hover:bg-primary/5 hover:text-primary transition-colors rounded-lg">
                        {cat.name}
                      </Link>
                    ))}
                  </div>
          ...
          <div className="hidden md:flex items-center gap-3">
            {user ? (
              <>
                <Link to="/dashboard" className="flex items-center gap-1 text-sm text-gray-700 hover:text-primary font-semibold transition-colors">
                  <span className="material-icons-round text-lg">person</span>
                  {firstName}
                </Link>
                <Link to="/profile" className="text-sm text-gray-500 hover:text-primary font-medium transition-colors" title="Account Settings">
                  <span className="material-icons-round text-lg">manage_accounts</span>
                </Link>
                <button onClick={handleLogout} className="text-sm border border-primary text-primary px-4 py-1.5 rounded-lg hover:bg-primary hover:text-white transition-colors font-semibold">
                  Logout
                </button>
              </>
            ) : (
              <Link to="/login" className="text-sm bg-primary text-white px-5 py-2 rounded-lg hover:bg-primary-dark transition-colors font-semibold">
                Login
              </Link>
            )}
          </div>
      ...
      <div className={`fixed top-0 right-0 h-full w-72 bg-white z-50 shadow-2xl transform transition-transform duration-300 md:hidden ${drawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <nav className="p-5 h-[calc(100vh-70px)] overflow-y-auto">
          <ul className="space-y-1 list-none m-0 p-0">
            <li>
              <NavLink to="/" end onClick={() => setDrawerOpen(false)}
                className={({ isActive }) => `block py-2.5 px-3 rounded-lg font-medium text-sm transition-colors ${isActive ? 'bg-primary/10 text-primary' : 'text-gray-700 hover:bg-gray-100'}`}>
                Home
              </NavLink>
            </li>
            ...
            <li>
              <div className="flex items-center justify-between">
                <NavLink to="/study-in-germany" onClick={() => setDrawerOpen(false)}
                  className={({ isActive }) => `flex-1 py-2.5 px-3 rounded-lg font-medium text-sm transition-colors ${isActive ? 'bg-primary/10 text-primary' : 'text-gray-700 hover:bg-gray-100'}`}>
                  Study
                </NavLink>
                <button onClick={() => setMobileStudyOpen(!mobileStudyOpen)} className="p-2 text-gray-500">
                  <span className={`material-icons-round transition-transform ${mobileStudyOpen ? 'rotate-180' : ''}`}>expand_more</span>
                </button>
              </div>
              {mobileStudyOpen && (
                <ul className="pl-4 mt-1 space-y-0.5 mb-2">
                  {STUDY_LINKS.map(item => (
                    <li key={item.to}>
                      <Link to={item.to} onClick={() => setDrawerOpen(false)}
                        className="flex items-center gap-2 py-2 px-3 text-sm text-gray-600 rounded-lg hover:bg-gray-50 hover:text-primary">
            ...
            <li>
              <div className="flex items-center justify-between">
                <span className="flex-1 py-2.5 px-3 rounded-lg font-medium text-sm text-gray-700">Packages</span>
            ...
            <li>
              <div className="flex items-center justify-between">
                <NavLink to="/products" onClick={() => setDrawerOpen(false)}
                  className={({ isActive }) => `flex-1 py-2.5 px-3 rounded-lg font-medium text-sm transition-colors ${isActive ? 'bg-primary/10 text-primary' : 'text-gray-700 hover:bg-gray-100'}`}>
                  Products
                </NavLink>
              ...
              {mobileProductsOpen && (
                <ul className="pl-6 mt-1 space-y-1 mb-2">
                  {categories.map(cat => (
                    <li key={cat.slug}>
                      <Link to={`/products?category=${cat.slug}`} onClick={() => setDrawerOpen(false)}
                        className="block py-2 px-3 text-sm text-gray-600 rounded-lg hover:bg-gray-50 hover:text-primary">
                        {cat.name}
                      </Link>
                    </li>
                  ))}
            ...
            <li><NavLink to="/blog" onClick={() => setDrawerOpen(false)} className={({ isActive }) => `block py-2.5 px-3 rounded-lg font-medium text-sm transition-colors ${isActive ? 'bg-primary/10 text-primary' : 'text-gray-700 hover:bg-gray-100'}`}>Blogs</NavLink></li>
            <li><NavLink to="/about" onClick={() => setDrawerOpen(false)} className={({ isActive }) => `block py-2.5 px-3 rounded-lg font-medium text-sm transition-colors ${isActive ? 'bg-primary/10 text-primary' : 'text-gray-700 hover:bg-gray-100'}`}>About</NavLink></li>
            <li><NavLink to="/contact" onClick={() => setDrawerOpen(false)} className={({ isActive }) => `block py-2.5 px-3 rounded-lg font-medium text-sm transition-colors ${isActive ? 'bg-primary/10 text-primary' : 'text-gray-700 hover:bg-gray-100'}`}>Contact</NavLink></li>
            {isAdmin && (
              <li>
                <NavLink to="/admin" onClick={() => setDrawerOpen(false)}
                  className={({ isActive }) => `block py-2.5 px-3 rounded-lg font-medium text-sm transition-colors ${isActive ? 'bg-primary/10 text-primary' : 'text-primary hover:bg-primary/10'}`}>
                  Admin Panel
                </NavLink>
              </li>
            )}
            ...
            {user ? (
              <>
                <Link to="/dashboard" onClick={() => setDrawerOpen(false)}
                  className="block w-full text-center py-2.5 px-4 border border-gray-300 rounded-lg text-gray-700 text-sm font-medium hover:bg-gray-50">
                  My Dashboard
                </Link>
                ...
                <button onClick={handleLogout}
                  className="block w-full text-center py-2.5 px-4 border border-primary text-primary rounded-lg text-sm font-medium hover:bg-primary hover:text-white transition-colors">
                  Logout
                </button>
              </>
            ) : (
              <Link to="/login" onClick={() => setDrawerOpen(false)}
                className="block w-full text-center py-2.5 px-4 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary-dark transition-colors">
                Login
              </Link>
            )}
          </div>
